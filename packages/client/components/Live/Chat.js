...
function Chat() {
  ...
  return (
    <>
      <div id="chat-feed" className="h-full overflow-y-scroll p-4 text-black">
        {msgs.length > 0 ? (
          msgs.map(m => (
            <div
              key={m.id}
              className="flex items-start w-full mb-5 relative text-black"
            >
              <Avatar
              ...
              <div className="flex flex-col flex-grow">
                ...
                <div className="pl-2 break-words w-[220px] mt-1">
                  {m.message}
                </div>
              </div>
          ...
          <div className="h-full flex justify-center items-center">
            <div className="w-full flex flex-col items-center text-center">
              ...
              <p>
                Welcome to the Aakriti. You can engage with the speaker and
...
function Badge({ isMod = false, isLocal }) {
  return (
    <div
      style={{
        border: `${isMod ? '1px solid transparent' : '1px solid #2f6eeb'}`,
      }}
      className="inline-flex items-center text-[10px] p-0.5 mx-1 rounded  text-foreground"
    >
      {isLocal ? (
        <>
